...
impl EmbeddedClient {
    ...
    ///
    /// Round-trip is verified end-to-end for the `Cosine` metric. The
    /// underlying `VectorIndex::load` hard-codes `CosineDistance` regardless
    /// of the metric recorded in `metadata.json`, so `L2` / `InnerProduct`
    /// indices load successfully but return cosine distances on search until
